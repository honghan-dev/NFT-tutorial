...
class RDFEntry {
    constructor(key, value, hasher = poseidon_1.DEFAULT_HASHER) {
        this.key = key;
        this.value = value;
        this.hasher = hasher;
        if (!key.parts.length) {
                        ...
                        switch (dataType) {
                            ...
                            case types_1.XSDNS.DateTime:
                                ...
                                if (dateRegEx.test(qoVal)) {
                                    value = temporal_polyfill_1.Temporal.Instant.from(new Date(qoVal).toISOString());
                                }
                                else {
                                    value = temporal_polyfill_1.Temporal.Instant.from(qoVal);
                                }
                                break;
                            default:
                                value = qoVal;
