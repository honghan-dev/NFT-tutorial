...
exports.jsonLdDocLoader = exports.getJsonLdDocLoader = exports.JsonLDLoader = void 0;
 ...
 */
class JsonLDLoader {
    constructor(secure = false, strictSSL = true, maxRedirects = -1, headers = {}) {
        this.secure = secure;
    ...
    async loadDocument(url, redirects = []) {
        ...
        if (this.secure && !isHttps) {
            throw new JsonLdError_1.default('URL could not be dereferenced; secure mode is enabled and ' +
                'the URL\'s scheme is not "https".', 'jsonld.InvalidUrl', { code: 'loading document failed', url });
        }
...
async function _fetch({ url, headers, strictSSL, httpAgent, httpsAgent }) {
    try {
        ...
        const isHttps = url.startsWith('https:');
        if (isHttps) {
            options.agent = httpsAgent || new https_1.default.Agent({ rejectUnauthorized: strictSSL });
        }
        else {
            if (httpAgent) {
...
exports.jsonLdDocLoader = (0, exports.getJsonLdDocLoader)();
