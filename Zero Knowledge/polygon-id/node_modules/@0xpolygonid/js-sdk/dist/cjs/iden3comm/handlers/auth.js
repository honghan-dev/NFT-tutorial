...
class AuthHandler {
    ...
    async handleAuthorizationRequestForGenesisDID(did, request) {
        ...
        const msgBytes = index_1.byteEncoder.encode(JSON.stringify(authResponse));
        const token = index_1.byteDecoder.decode(await this._packerMgr.pack(constants_1.MediaType.ZKPMessage, msgBytes, {
            senderDID: did,
            profileNonce: 0,
    ...
    async generateAuthorizationResponse(userGenesisDID, authProfileNonce, authRequest, zkpRequestsWithCreds) {
        ...
        for (const r of zkpRequestsWithCreds) {
            const { proof } = await this._proofService.generateProof(r.req, userGenesisDID, r.credential, {
