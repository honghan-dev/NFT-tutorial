...
const envelopeToProtocolMessage = async (e) => {
    const t = await js_jwz_1.Token.parse(index_1.byteDecoder.decode(e));
    const pBytes = index_1.byteEncoder.encode(t.getPayload());
...
const bytesToProtocolMessage = (bytes) => {
    const str = index_1.byteDecoder.decode(bytes);
...
const bytesToEnvelopeStub = (envelope) => {
    ...
    const str = index_1.byteDecoder.decode(envelope);
    const message = JSON.parse(str);
