...
const revocation_1 = require("../credentials/revocation");
/**
 * @public
...
class IdentityWallet {
    ...
    async createIdentity(hostUrl, opts) {
        ...
        if (!opts) {
            opts = {
                ...
                blockchain: js_iden3_core_1.Blockchain.Polygon,
                networkId: js_iden3_core_1.NetworkId.Mumbai
            };
        }
        ...
        const currentState = await (0, js_merkletree_1.hashElems)([
    ...
    async issueCredential(issuerDID, req, hostUrl, opts) {
        ...
        hostUrl = hostUrl.replace(/\/$/, '');
        const schema = await new loaders_1.UniversalSchemaLoader('ipfs.io').load(req.credentialSchema);
        ...
        const coreClaim = await new schema_processor_1.Parser().parseClaim(credential, `${jsonSchema.$metadata.uris['jsonLdContext']}#${req.type}`, schema, coreClaimOpts);
    ...
    async addCredentialsToMerkleTree(credentials, issuerDID) {
        ...
        await this._storage.mt.addToMerkleTree(issuerDID.toString(), mt_1.MerkleTreeType.Roots, newIssuerTreeState.claimsTree.root.bigInt(), BigInt(0));
    ...
    async publishStateToRHS(issuerDID, rhsURL, revokedNonces) {
        ...
        await (0, revocation_1.pushHashesToRHS)(treeState.state, {
